...
// sum mod function pour reduction
u32 sum_mod_fun(u32 out, u32 in)
{
        out = (in + out) % prime;
        return out;
}

// redefinition reduction
#pragma omp declare reduction(redu:u32                                  \
...
/* c = a*b */
// produit normal
u32 *matmulAB(u32 const *A, u32 const *B)
{
        u32 *res = malloc(sizeof(*res) * (n * n));
        for (int i = 0; i < n * n; i++)
        {
                res[i] = 0;
        }
...
/* c += at*b */
// produit normal
u32 *matmulAtB(u32 const *A, u32 const *B)
{
        u32 *res = malloc(sizeof(*res) * (n * n));
        for (int i = 0; i < n * n; i++)
        {
...
#pragma omp parallel for reduction(redu \
        ...
        for (int i = 0; i < N; i += n)
        {
                u32 *test = matmulAtB(&v[i * n], &Av[i * n]);
                for (int j = 0; j < n * n; j++)
                {
                        vtAv[j] += test[j];
                }
...
#pragma omp parallel for reduction(redu \
        ...
        for (int i = 0; i < N; i += n)
        {
                u32 *test = matmulAtB(&Av[i * n], &Av[i * n]);
                for (int j = 0; j < n * n; j++)
                {
                        vtAAv[j] += test[j];
                }
...
void orthogonalize(u32 *v, u32 *tmp, u32 *p, u32 *d, u32 const *vtAv, const u32 *vtAAv,
                   u32 const *winv, int N, u32 const *Av, long sizeoftmp)
{
...
u32 *block_lanczos(struct sparsematrix_t *M, int n, int my_rank, int nb_processus)
{
        ...
        while (true)
        {
                ...
                orthogonalize(v_processus, tmp_processus, p, d, vtAv_processus, vtAAv_processus, winv, M_processus.ncols, Av_processus, block_size_pad);
