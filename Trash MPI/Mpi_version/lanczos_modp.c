...
int partition(struct sparsematrix_t *M, int low, int high)
{
        ...
        swap(&M->j[i + 1], &M->j[high]);
        swap(&M->i[i + 1], &M->i[high]);
        swapX(&M->x[i + 1], &M->x[high]);

        return (i + 1);
}

void quickSort(struct sparsematrix_t *M, int low, int high)
{
        if (low < high)
        {
