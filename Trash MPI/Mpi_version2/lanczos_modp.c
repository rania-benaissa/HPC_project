...
int partition(struct sparsematrix_t *M, int low, int high, int order)
{
        u32 temp;
        if (order == 0)
        {

                int pivot = M->j[high]; // pivot
...
void quickSort(struct sparsematrix_t *M, int low, int high, int order)
{
        if (low < high)
        {
                /* pi is partitioning index */
                int pi = partition(M, low, high, order);

                // Separately sort elements before
                // partition and after partition
                quickSort(M, low, pi - 1, order);
                quickSort(M, pi + 1, high, order);
        }
}
...
void swap(int neighbor_rank, int nb_processus, struct sparsematrix_t *M_processus, int nnz_neighbor, int max)
{
        ...
        if (neighbor_rank >= 0 && neighbor_rank <= nb_processus - 1)
        {
                ...
                quickSort(&combo, 0, combo.nnz - 1, max);
...
void odd_even_sort(struct sparsematrix_t *M_processus, int ncols, int my_rank, int nb_processus)
{
        ...
        {
                quickSort(M_processus, 0, M_processus->nnz - 1, 0);
...
u32 *block_lanczos(struct sparsematrix_t const M, int n, int my_rank, int nb_processus)
{
        ...
        // printf("Version 2 Terminated in %f\n", wtime() - time);
