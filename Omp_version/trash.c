...
u64 sum_mod_fun(u64 out, u64 in)
{
        out = (in + out) % prime;
        return out;
}

#pragma omp declare reduction(redu:u64                                  \
                              : omp_out = sum_mod_fun(omp_out, omp_in)) \
    initializer(omp_priv = {0})

/* y += M*x or y += transpose(M)*x, according to the transpose flag */
void sparse_matrix_vector_product(u64 *y, struct sparsematrix_t const *M, u64 const *x, bool transpose)
{
        long nnz = M->nnz;
        ...
        // local y
        // u64 *local_y = malloc(sizeof(*local_y) * (nrows*n));
        printf("init \n");
...
#pragma omp parallel for reduction(redu \
                                   : y[:nrows * n])
        for (long k = 0; k < nnz; k++)
        {
                printf("reduction for\n");
...
void matmul_CpAB(u64 *C, u64 const *A, u64 const *B)
{
...
void matmul_CpAtB(u64 *C, u64 const *A, u64 const *B)
{
        for (int i = 0; i < n; i++)
                for (int j = 0; j < n; j++)
                        for (int k = 0; k < n; k++)
...
/* c += a*b */
u64 *matmulAB(u64 const *A, u64 const *B)
{
        u64 *res = malloc(sizeof(*res) * (n * n));
        for (int i = 0; i < n * n; i++)
        {
                res[i] = 0;
        }
        ...
        for (int i = 0; i < n; i++)
                for (int j = 0; j < n; j++)
                        for (int k = 0; k < n; k++)
                        {
                                ...
                                u64 y = A[i * n + k];
                                u64 z = B[k * n + j];
                                res[i * n + j] = (x + y * z) % prime;
                        }
        return res;
}

/* c += at*b */
u64 *matmulAtB(u64 const *A, u64 const *B)
{
        u64 *res = malloc(sizeof(*res) * (n * n));
        for (int i = 0; i < n * n; i++)
        {
 ...
 */
int semi_inverse(u64 const *M_, u64 *winv, u64 *d)
{
        u64 M[n * n];
        int npiv = 0;
...
void block_dot_products(u64 *vtAv, u64 *vtAAv, int N, u64 const *Av, u64 const *v)
{
        for (int i = 0; i < n * n; i++)
        {
                vtAv[i] = 0;
...
#pragma omp parallel for reduction(redu \
        ...
        for (int i = 0; i < N; i += n)
        {
                u64 *test = matmulAtB(&v[i * n], &Av[i * n]);
                for (int j = 0; j < n * n; j++)
                {
                        vtAv[j] += test[j];
                }
...
#pragma omp parallel for reduction(redu \
        ...
        for (int i = 0; i < N; i += n)
        {
                u64 *test = matmulAtB(&Av[i * n], &Av[i * n]);
                for (int j = 0; j < n * n; j++)
                {
                        vtAAv[j] += test[j];
                }
...
void orthogonalize(u64 *v, u64 *tmp, u64 *p, u64 *d, u64 const *vtAv, const u64 *vtAAv,
                   u64 const *winv, int N, u64 const *Av)
{
...
/* optional tests */
void correctness_tests(u64 const *vtAv, u64 const *vtAAv, u64 const *winv, u64 const *d)
{
...
void final_check(int nrows, int ncols, u64 const *v, u64 const *vtM)
{
        printf("Final check:\n");
        /* Check if v != 0 */
        bool good = false;
...
u64 *block_lanczos(struct sparsematrix_t const *M, int n, bool transpose)
{
        ...
        u64 *v = malloc(sizeof(*v) * block_size_pad);
        u64 *tmp = malloc(sizeof(*tmp) * block_size_pad);
        u64 *Av = malloc(sizeof(*Av) * block_size_pad);
        u64 *p = malloc(sizeof(*p) * block_size_pad);
...
void save_vector_block(char const *filename, int nrows, int ncols, u64 const *v)
{
...
int main(int argc, char **argv)
{
        ...
        u64 *kernel = block_lanczos(&M, n, right_kernel);
